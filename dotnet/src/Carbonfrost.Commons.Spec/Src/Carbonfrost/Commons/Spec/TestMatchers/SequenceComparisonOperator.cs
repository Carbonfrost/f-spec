...
namespace Carbonfrost.Commons.Spec.TestMatchers {
    ...
    abstract class SequenceComparisonOperator : ISequenceComparisonOperator {
        ...
        public void Apply<T>(IExpectation<IEnumerable<T>> e, params T[] expected) {
            Apply<T>(e, expected, message: (string) null);
        }

        public void Apply<T>(IExpectation<IEnumerable<T>> e, IEnumerable<T> expected, IEqualityComparer<T> comparer) {
            Apply<T>(e, expected, comparer, message: null);
        }

        public void Apply<T>(IExpectation<IEnumerable<T>> e, IEnumerable<T> expected, Comparison<T> comparison) {
            Apply<T>(e, expected, comparison, message: null);
        }

        public void Apply<T>(IExpectation<IEnumerable<T>> e, IEnumerable<T> expected) {
            Apply<T>(e, expected, message: (string) null);
        }

        public void Apply<T>(IExpectation<IEnumerable<T>> e, IEnumerable<T> expected, IEqualityComparer<T> comparer, string message, params object[] args) {
            e.Like(CreateMatcher(expected, comparer), message, (object[]) args);
        }

        public void Apply<T>(IExpectation<IEnumerable<T>> e, IEnumerable<T> expected, Comparison<T> comparison, string message, params object[] args) {
