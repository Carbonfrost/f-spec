...
namespace Carbonfrost.Commons.Spec.TestMatchers {
    ...
    abstract class ComparisonOperator : IComparisonOperator {
        ...
        public void Apply<T>(Expectation<T> e, T expected) {
            Apply(e, expected, (string) null);
        }

        public void Apply<T>(Expectation<T> e, T expected, IComparer<T> comparer) {
            Apply(e, expected, comparer, null);
        }

        public void Apply<T>(Expectation<T> e, T expected, Comparison<T> comparison) {
            Apply(e, expected, comparison, null);
        }

        public void Apply<T>(Expectation<T> e, T expected, string message, params object[] args) {
            e.Should(CreateMatcher(expected), message, (object[]) args);
        }

        public void Apply<T>(Expectation<T> e, T expected, IComparer<T> comparer, string message, params object[] args) {
            e.Should(CreateMatcher(expected, comparer), message, (object[]) args);
        }

        public void Apply<T>(Expectation<T> e, T expected, Comparison<T> comparison, string message, params object[] args) {
